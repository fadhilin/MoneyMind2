...
const incomeCategories = [
  ...
  { name: "Narik/Dagang", icon: "store", color: "blue-500" },
  ...
  { name: "Lain-lain", icon: "more_horiz", color: "slate-500" },
];
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
          <div className="px-6 py-10 mt-6 text-center bg-black/5 dark:bg-white/5 border border-slate-200 dark:border-white/5 mx-6 rounded-3xl mb-8 select-none">
            ...
            <div
              className={`text-4xl sm:text-6xl font-black flex justify-center items-baseline gap-2 transition-colors ${amount !== "0" ? "text-white" : "text-slate-400 dark:text-slate-500"}`}
            >
            ...
            <div className="grid grid-cols-3 sm:grid-cols-4 gap-4 md:gap-6">
              {displayCategories.map((cat) => (
                ...
                >
                  ...
                  <span
                    className={`text-[10px] md:text-xs font-bold text-center leading-tight transition-colors w-full truncate px-1 ${category === cat.name ? "text-primary" : "text-slate-600 dark:text-slate-400"}`}
                  >
                    {cat.name}
                  </span>
                </button>
