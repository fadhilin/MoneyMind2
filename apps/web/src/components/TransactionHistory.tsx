    ...
    <div className="glass-card p-6 md:p-8 rounded-3xl md:rounded-4xl bg-white dark:bg-black border border-slate-200 dark:border-white/10 shadow-sm">
      ...
      <div className="space-y-4">
        {recentTransactions.map((tx) => (
          <div
            key={tx.id}
            className="flex items-center justify-between p-3 md:p-4 rounded-2xl hover:bg-slate-50 dark:hover:bg-white/5 transition-all group"
          >
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="font-bold text-sm text-black dark:text-white truncate max-w-25 sm:max-w-37.5">
                  {tx.note || tx.category}
                </p>
            ...
            <div className="text-right">
              <p
                className={`font-bold text-sm ${tx.type === "income" ? "text-emerald-500" : "text-rose-500"}`}
              >
                {tx.type === "income" ? "+" : "-"}Rp{" "}
        ...
        {transactions.length === 0 && (
          <div className="py-12 flex flex-col items-center justify-center text-center opacity-60">
            ...
            <p className="text-xs text-slate-400 mt-1 max-w-50">
              Catat pengeluaran atau pemasukan hari ini.
            </p>
          </div>
        )}
      </div>
