...
export async function getBudgets(month?: string, date?: string): Promise<Budget[]> {
  ...
  if (month) {
    ...
    if (!hasMakan) {
      const newM = await createBudget({ category: 'Makan & Minum', icon: 'restaurant', color: 'orange-500', date: month + '-01' });
      budgets.push(newM);
    }
    if (!hasTransport) {
      const newT = await createBudget({ category: 'Transportasi', icon: 'directions_car', color: 'blue-500', date: month + '-01' });
      budgets.push(newT);
    }
  }
  ...
  for (const b of budgets) {
    ...
    const expense = relatedTx.filter(t => t.type === 'expense').reduce((sum, t) => sum + t.amount, 0);
    const income = relatedTx.filter(t => t.type === 'income').reduce((sum, t) => sum + t.amount, 0);
...
export async function deleteBudget(id: string, month?: string): Promise<Budget> {
  const b = await db.budgets.get(id);
  if (!b) throw new Error('Budget not found');
  ...
  const txs: Transaction[] = await db.transactions
    .filter(t => t.category === b.category && t.date.startsWith(month))
    .toArray();
...
export async function depositBudget(id: string, amount: number, date?: string): Promise<void> {
  ...
  await db.transactions.add({
    ...
    amount: amount,
    category: b.category,
    date: date && date.length === 10 ? `${date}T${new Date().toISOString().split('T')[1]}` : (date || new Date().toISOString()),
