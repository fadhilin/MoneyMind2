...
const Budget: React.FC = () => {
  ...
  const [draftUpdates, setDraftUpdates] = useState<Record<string, { category?: string; limitAmount?: number }>>({});
...
const listIcons = [
  ...
  const handleTxSubmit = () => {
    ...
    if (txType === 'deposit') {
      ...
      withdrawBudget.mutate({ id: selectedBudget.id, amount, date: txDate }, {
  ...
  return (
    <div className="max-w-6xl mx-auto pb-20 px-4 animate-in fade-in zoom-in-95 duration-500">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8 md:mb-10">
        <div>
          <h2 className="text-2xl md:text-3xl font-bold tracking-tight text-black dark:text-white">Budget Harian</h2>
          <p className="text-xs md:text-sm text-slate-500 dark:text-slate-400 mt-1">
            {new Date(globalDate).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}
          </p>
        </div>
        <div className="flex flex-col sm:flex-row items-center gap-4">
          <div className="flex w-full sm:w-auto gap-4 bg-slate-50 dark:bg-white/5 py-2 px-4 rounded-xl border border-slate-200 dark:border-white/10 shadow-sm justify-between sm:justify-end">
            <div className="text-right">
              <p className="text-[9px] md:text-[10px] uppercase font-bold text-slate-500 dark:text-slate-400">Total Saldo</p>
              <p className="text-xs md:text-sm font-black text-black dark:text-white">Rp {monthlyIncome.toLocaleString('id-ID')}</p>
            </div>
            <div className="w-px bg-slate-200 dark:bg-white/10 self-stretch my-1"></div>
            <div className="text-right">
              <p className="text-[9px] md:text-[10px] uppercase font-bold text-slate-500 dark:text-slate-400">Sisa Saldo</p>
              <p className={`text-xs md:text-sm font-black ${remainingBudget < 0 ? 'text-rose-500' : 'text-black dark:text-white'}`}>Rp {remainingBudget.toLocaleString('id-ID')}</p>
            </div>
          </div>
          <div className="flex items-center gap-2 w-full sm:w-auto">
            ...
            >
              <span className="material-symbols-outlined text-base md:text-lg">{isEditing ? 'check' : 'edit'}</span>
      ...
      <section className="mb-8 md:mb-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        <div className={`md:col-span-2 lg:col-span-3 glass rounded-3xl md:rounded-4xl p-6 md:p-8 grid grid-cols-2 items-center border-l-4 md:border-l-8 ${remainingBudget < 0 ? 'border-l-rose-500 bg-rose-500/5' : 'border-l-primary bg-primary/5'} shadow-xl md:shadow-2xl shadow-primary/5 relative group/header`}>
          ...
          <div className="flex flex-col items-center justify-start text-center relative px-2 md:px-6 lg:px-10 h-full">
            <div className="h-4 flex items-center justify-center mb-4 md:mb-6 relative w-full">
              <p className="text-[10px] md:text-[12px] font-bold text-primary dark:text-primary uppercase tracking-widest leading-none whitespace-nowrap">Masuk</p>
              {realIncome > 0 && (
                <button
                  onClick={() => { if (window.confirm('Hapus seluruh data keuangan hari ini?')) deleteByDate.mutate(globalDate); }}
            ...
            <div className="h-12 md:h-16 flex items-center justify-center -mt-2">
              <h3 className="text-xl md:text-2xl lg:text-3xl font-black text-primary leading-none tracking-tight flex items-center gap-1 md:gap-2">
                <span className="text-sm md:text-xl lg:text-3xl text-primary font-bold">Rp</span>
          ...
          <div className="flex flex-col items-center justify-start text-center relative px-2 md:px-6 lg:px-10 h-full">
            <div className="h-4 flex items-center justify-center mb-4 md:mb-6">
              <p className="text-[10px] md:text-[12px] font-bold text-rose-500 dark:text-rose-400 uppercase tracking-widest leading-none">Keluar</p>
            </div>
            <div className="h-12 md:h-16 flex items-center justify-center -mt-2">
              <p className="text-xl md:text-2xl lg:text-3xl font-black text-rose-500 leading-none flex items-center gap-1 md:gap-2">
                <span className="text-sm md:text-xl lg:text-3xl font-bold">Rp</span>
        ...
        <div className="glass rounded-3xl md:rounded-4xl p-6 md:p-8 bg-linear-to-br from-primary/10 to-transparent border border-primary/20 flex flex-col justify-center">
          <div className="flex items-center gap-2 mb-2 md:mb-3">
            <span className="material-symbols-outlined text-primary text-xs md:text-sm">shield</span>
            <p className="text-[10px] md:text-xs font-bold text-slate-500 dark:text-slate-400 uppercase">Safety Spend</p>
          </div>
          <h3 className="text-lg md:text-xl lg:text-2xl font-black text-black dark:text-white flex items-baseline gap-1 md:gap-1.5">
            <span className="text-sm md:text-lg lg:text-xl font-bold text-black dark:text-white">Rp</span>
            {safetySpend.toLocaleString('id-ID')}
          </h3>
          <p className="text-[9px] md:text-[10px] text-slate-400 mt-2 italic">*Batas aman hari ini</p>
        </div>
      </section>
      ...
      {isLoading ? (
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-10">
          {[...budgets].sort((a, b) => {
            ...
            return (
              <div key={b.id} className="glass-card p-4 md:p-6 rounded-2xl md:rounded-3xl flex flex-col h-full gap-3 md:gap-4 border-t-4 bg-white dark:bg-black border-slate-200 dark:border-white/10 relative group shadow-sm transition-all hover:shadow-md">
                ...
                <div className="flex-1">
                  {isEditing && b.category !== 'Makan & Minum' && b.category !== 'Transportasi' ? (
                    ...
                    <h4 className="text-base md:text-lg font-bold text-black dark:text-white truncate">{b.category}</h4>
                  )}
                  <p className="text-slate-500 dark:text-slate-400 text-[10px] md:text-xs line-clamp-1">{b.description}</p>
                </div>
                <div className="space-y-2 mt-auto">
                  <div className="flex justify-between text-[10px] md:text-xs font-bold gap-2">
                    <span className="text-slate-400 leading-tight">Terpakai: <br/>Rp {b.spent.toLocaleString('id-ID')}</span>
                    {isEditing ? (
                      <input
                        type="number"
                      ...
                      <span className="text-black dark:text-white text-right leading-tight">Batas: <br/>Rp {b.limit.toLocaleString('id-ID')}</span>
                    )}  
                  </div>
                  <div className="h-2 md:h-3 w-full bg-slate-100 dark:bg-white/5 rounded-full overflow-hidden">
                    <div className={`h-full bg-primary rounded-full transition-all duration-700 ${percent > 90 ? 'bg-rose-500' : ''}`} style={{ width: `${percent}%` }}></div>
                  </div>
                  {!isEditing && (
          ...
          <div className="glass-card rounded-2xl md:rounded-3xl p-5 md:p-6 border-2 border-dashed border-slate-200 dark:border-white/10 flex flex-col items-center justify-center gap-2 md:gap-3 bg-transparent hover:bg-primary/5 hover:border-primary transition-all group h-full min-h-[220px] md:min-h-[250px]">
            ...
            <div ref={dropdownRef} className="relative w-full mx-auto mb-1">
              ...
              >
                <span className="material-symbols-outlined text-base md:text-lg">{newCatIcon}</span>
              ...
              {isDropdownOpen && (
                <div className="absolute top-full left-0 w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-white/10 shadow-lg py-2 z-50 max-h-40 overflow-y-auto">
                  {listIcons.map((item) => (
                    ...
                    >
                      <span className="material-symbols-outlined text-sm">{item.id}</span>
                      <span className="capitalize font-medium text-xs">{item.label}</span>
                    </button>
                  ))}
                </div>
              )}
          ...
          <input
            ...
            value={newCatLimit}
            onChange={(e: React.ChangeEvent<HTMLInputElement>) => setNewCatLimit(e.target.value)}
          />
            <button
            ...
            >
              <span className="material-symbols-outlined text-xs md:text-sm">add</span> Simpan
            </button>
          </div>
        </section>
      )}
      ...
      {isTxModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white dark:bg-[#151121] w-full max-w-md rounded-3xl p-8 shadow-2xl border border-slate-200 dark:border-white/10">
            ...
            <p className="text-sm text-slate-500 mb-6">Kategori: <span className="font-bold text-primary">{selectedBudget?.category}</span></p>
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  autoFocus
                  value={txAmount}
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => setTxAmount(e.target.value)}
